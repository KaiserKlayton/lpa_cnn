#include "helper/reader.h"
#include "helper/writer.h"
#include "../layers/convolution_layer/convolution.h"
#include "../layers/pooling_layer/pooling.h"
#include "../layers/fully_connected_layer/fully_connected.h"
#include "../layers/relu_layer/relu.h"
#include "../layers/eltwise_layer/eltwise.h"

using Eigen::Matrix;
using Eigen::RowMajor;

int main() 
{
	const int im_height_1 = 224;
	const int im_width_1 = 224;
	const int im_depth_1 = 3;
	const int im_size_1 = im_height_1*im_width_1;
	
	const int k_num_1 = 64;
	const int k_size_1 = 9;
	const int stride_1 = 1;
	const int k_depth_1 = im_depth_1;
	
	const int p1_1 = 1;
	const int p2_1 = 1;
	
	const int output_height_1 = (((im_height_1+(2*p1_1)) - sqrt(k_size_1))/stride_1) + 1;
	const int output_width_1 = (((im_width_1+(2*p2_1)) - sqrt(k_size_1))/stride_1) + 1;
	const int output_size_1 = output_height_1 * output_width_1;
	
	MatrixXd conv1_1_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv1_1_weights.csv");
	Map<MatrixXd> conv1_1_w(conv1_1_weights.data(), k_num_1, k_size_1 * k_depth_1);
	
	MatrixXd conv1_1_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv1_1_biases.csv");
	VectorXd conv1_1_b(Map<VectorXd>(conv1_1_biases.data(), conv1_1_biases.cols()*conv1_1_biases.rows()));
	
	const int im_height_2 = output_height_1;
	const int im_width_2 = output_width_1;
	const int im_depth_2 = k_num_1;
	const int im_size_2 = im_height_2 * im_width_2;
	
	const int k_num_2 = 64;
	const int k_size_2 = 9;
	const int stride_2 = 1;
	const int k_depth_2 = im_depth_2;
	
	const int p1_2 = 1;
	const int p2_2 = 1;
	
	const int output_height_2 = (((im_height_2+(2*p1_2)) - sqrt(k_size_2))/stride_2) + 1;
	const int output_width_2 = (((im_width_2+(2*p2_2)) - sqrt(k_size_2))/stride_2) + 1;
	const int output_size_2 = output_height_2 * output_width_2;
	
	MatrixXd conv1_2_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv1_2_weights.csv");
	MatrixXd conv1_2_w = conv1_2_weights;
	
	MatrixXd conv1_2_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv1_2_biases.csv");
	VectorXd conv1_2_b(Map<VectorXd>(conv1_2_biases.data(), conv1_2_biases.cols()*conv1_2_biases.rows()));
	
	const int f_1 = 2;
	const int s_1 = 2;
	
	const int pp1_1 = 0;
	const int pp2_1 = 0;
	
	const int im_height_3 = ((output_height_2 - f_1 + 2 * pp1_1) / s_1) + 1;
	const int im_width_3 = ((output_width_2 - f_1 + 2 * pp2_1) / s_1) + 1;
	const int im_depth_3 = k_num_2;
	const int im_size_3 = im_height_3 * im_width_3;
	
	const int k_num_3 = 128;
	const int k_size_3 = 9;
	const int stride_3 = 1;
	const int k_depth_3 = im_depth_3;
	
	const int p1_3 = 1;
	const int p2_3 = 1;
	
	const int output_height_3 = (((im_height_3+(2*p1_3)) - sqrt(k_size_3))/stride_3) + 1;
	const int output_width_3 = (((im_width_3+(2*p2_3)) - sqrt(k_size_3))/stride_3) + 1;
	const int output_size_3 = output_height_3 * output_width_3;
	
	MatrixXd conv2_1_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv2_1_weights.csv");
	MatrixXd conv2_1_w = conv2_1_weights;
	
	MatrixXd conv2_1_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv2_1_biases.csv");
	VectorXd conv2_1_b(Map<VectorXd>(conv2_1_biases.data(), conv2_1_biases.cols()*conv2_1_biases.rows()));
	
	const int im_height_4 = output_height_3;
	const int im_width_4 = output_width_3;
	const int im_depth_4 = k_num_3;
	const int im_size_4 = im_height_4 * im_width_4;
	
	const int k_num_4 = 128;
	const int k_size_4 = 9;
	const int stride_4 = 1;
	const int k_depth_4 = im_depth_4;
	
	const int p1_4 = 1;
	const int p2_4 = 1;
	
	const int output_height_4 = (((im_height_4+(2*p1_4)) - sqrt(k_size_4))/stride_4) + 1;
	const int output_width_4 = (((im_width_4+(2*p2_4)) - sqrt(k_size_4))/stride_4) + 1;
	const int output_size_4 = output_height_4 * output_width_4;
	
	MatrixXd conv2_2_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv2_2_weights.csv");
	MatrixXd conv2_2_w = conv2_2_weights;
	
	MatrixXd conv2_2_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv2_2_biases.csv");
	VectorXd conv2_2_b(Map<VectorXd>(conv2_2_biases.data(), conv2_2_biases.cols()*conv2_2_biases.rows()));
	
	const int f_2 = 2;
	const int s_2 = 2;
	
	const int pp1_2 = 0;
	const int pp2_2 = 0;
	
	const int im_height_5 = ((output_height_4 - f_2 + 2 * pp1_2) / s_2) + 1;
	const int im_width_5 = ((output_width_4 - f_2 + 2 * pp2_2) / s_2) + 1;
	const int im_depth_5 = k_num_4;
	const int im_size_5 = im_height_5 * im_width_5;
	
	const int k_num_5 = 256;
	const int k_size_5 = 9;
	const int stride_5 = 1;
	const int k_depth_5 = im_depth_5;
	
	const int p1_5 = 1;
	const int p2_5 = 1;
	
	const int output_height_5 = (((im_height_5+(2*p1_5)) - sqrt(k_size_5))/stride_5) + 1;
	const int output_width_5 = (((im_width_5+(2*p2_5)) - sqrt(k_size_5))/stride_5) + 1;
	const int output_size_5 = output_height_5 * output_width_5;
	
	MatrixXd conv3_1_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_1_weights.csv");
	MatrixXd conv3_1_w = conv3_1_weights;
	
	MatrixXd conv3_1_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_1_biases.csv");
	VectorXd conv3_1_b(Map<VectorXd>(conv3_1_biases.data(), conv3_1_biases.cols()*conv3_1_biases.rows()));
	
	const int im_height_6 = output_height_5;
	const int im_width_6 = output_width_5;
	const int im_depth_6 = k_num_5;
	const int im_size_6 = im_height_6 * im_width_6;
	
	const int k_num_6 = 256;
	const int k_size_6 = 9;
	const int stride_6 = 1;
	const int k_depth_6 = im_depth_6;
	
	const int p1_6 = 1;
	const int p2_6 = 1;
	
	const int output_height_6 = (((im_height_6+(2*p1_6)) - sqrt(k_size_6))/stride_6) + 1;
	const int output_width_6 = (((im_width_6+(2*p2_6)) - sqrt(k_size_6))/stride_6) + 1;
	const int output_size_6 = output_height_6 * output_width_6;
	
	MatrixXd conv3_2_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_2_weights.csv");
	MatrixXd conv3_2_w = conv3_2_weights;
	
	MatrixXd conv3_2_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_2_biases.csv");
	VectorXd conv3_2_b(Map<VectorXd>(conv3_2_biases.data(), conv3_2_biases.cols()*conv3_2_biases.rows()));
	
	const int im_height_7 = output_height_6;
	const int im_width_7 = output_width_6;
	const int im_depth_7 = k_num_6;
	const int im_size_7 = im_height_7 * im_width_7;
	
	const int k_num_7 = 256;
	const int k_size_7 = 9;
	const int stride_7 = 1;
	const int k_depth_7 = im_depth_7;
	
	const int p1_7 = 1;
	const int p2_7 = 1;
	
	const int output_height_7 = (((im_height_7+(2*p1_7)) - sqrt(k_size_7))/stride_7) + 1;
	const int output_width_7 = (((im_width_7+(2*p2_7)) - sqrt(k_size_7))/stride_7) + 1;
	const int output_size_7 = output_height_7 * output_width_7;
	
	MatrixXd conv3_3_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_3_weights.csv");
	MatrixXd conv3_3_w = conv3_3_weights;
	
	MatrixXd conv3_3_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_3_biases.csv");
	VectorXd conv3_3_b(Map<VectorXd>(conv3_3_biases.data(), conv3_3_biases.cols()*conv3_3_biases.rows()));
	
	const int im_height_8 = output_height_7;
	const int im_width_8 = output_width_7;
	const int im_depth_8 = k_num_7;
	const int im_size_8 = im_height_8 * im_width_8;
	
	const int k_num_8 = 256;
	const int k_size_8 = 9;
	const int stride_8 = 1;
	const int k_depth_8 = im_depth_8;
	
	const int p1_8 = 1;
	const int p2_8 = 1;
	
	const int output_height_8 = (((im_height_8+(2*p1_8)) - sqrt(k_size_8))/stride_8) + 1;
	const int output_width_8 = (((im_width_8+(2*p2_8)) - sqrt(k_size_8))/stride_8) + 1;
	const int output_size_8 = output_height_8 * output_width_8;
	
	MatrixXd conv3_4_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_4_weights.csv");
	MatrixXd conv3_4_w = conv3_4_weights;
	
	MatrixXd conv3_4_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv3_4_biases.csv");
	VectorXd conv3_4_b(Map<VectorXd>(conv3_4_biases.data(), conv3_4_biases.cols()*conv3_4_biases.rows()));
	
	const int f_3 = 2;
	const int s_3 = 2;
	
	const int pp1_3 = 0;
	const int pp2_3 = 0;
	
	const int im_height_9 = ((output_height_8 - f_3 + 2 * pp1_3) / s_3) + 1;
	const int im_width_9 = ((output_width_8 - f_3 + 2 * pp2_3) / s_3) + 1;
	const int im_depth_9 = k_num_8;
	const int im_size_9 = im_height_9 * im_width_9;
	
	const int k_num_9 = 512;
	const int k_size_9 = 9;
	const int stride_9 = 1;
	const int k_depth_9 = im_depth_9;
	
	const int p1_9 = 1;
	const int p2_9 = 1;
	
	const int output_height_9 = (((im_height_9+(2*p1_9)) - sqrt(k_size_9))/stride_9) + 1;
	const int output_width_9 = (((im_width_9+(2*p2_9)) - sqrt(k_size_9))/stride_9) + 1;
	const int output_size_9 = output_height_9 * output_width_9;
	
	MatrixXd conv4_1_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_1_weights.csv");
	MatrixXd conv4_1_w = conv4_1_weights;
	
	MatrixXd conv4_1_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_1_biases.csv");
	VectorXd conv4_1_b(Map<VectorXd>(conv4_1_biases.data(), conv4_1_biases.cols()*conv4_1_biases.rows()));
	
	const int im_height_10 = output_height_9;
	const int im_width_10 = output_width_9;
	const int im_depth_10 = k_num_9;
	const int im_size_10 = im_height_10 * im_width_10;
	
	const int k_num_10 = 512;
	const int k_size_10 = 9;
	const int stride_10 = 1;
	const int k_depth_10 = im_depth_10;
	
	const int p1_10 = 1;
	const int p2_10 = 1;
	
	const int output_height_10 = (((im_height_10+(2*p1_10)) - sqrt(k_size_10))/stride_10) + 1;
	const int output_width_10 = (((im_width_10+(2*p2_10)) - sqrt(k_size_10))/stride_10) + 1;
	const int output_size_10 = output_height_10 * output_width_10;
	
	MatrixXd conv4_2_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_2_weights.csv");
	MatrixXd conv4_2_w = conv4_2_weights;
	
	MatrixXd conv4_2_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_2_biases.csv");
	VectorXd conv4_2_b(Map<VectorXd>(conv4_2_biases.data(), conv4_2_biases.cols()*conv4_2_biases.rows()));
	
	const int im_height_11 = output_height_10;
	const int im_width_11 = output_width_10;
	const int im_depth_11 = k_num_10;
	const int im_size_11 = im_height_11 * im_width_11;
	
	const int k_num_11 = 512;
	const int k_size_11 = 9;
	const int stride_11 = 1;
	const int k_depth_11 = im_depth_11;
	
	const int p1_11 = 1;
	const int p2_11 = 1;
	
	const int output_height_11 = (((im_height_11+(2*p1_11)) - sqrt(k_size_11))/stride_11) + 1;
	const int output_width_11 = (((im_width_11+(2*p2_11)) - sqrt(k_size_11))/stride_11) + 1;
	const int output_size_11 = output_height_11 * output_width_11;
	
	MatrixXd conv4_3_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_3_weights.csv");
	MatrixXd conv4_3_w = conv4_3_weights;
	
	MatrixXd conv4_3_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_3_biases.csv");
	VectorXd conv4_3_b(Map<VectorXd>(conv4_3_biases.data(), conv4_3_biases.cols()*conv4_3_biases.rows()));
	
	const int im_height_12 = output_height_11;
	const int im_width_12 = output_width_11;
	const int im_depth_12 = k_num_11;
	const int im_size_12 = im_height_12 * im_width_12;
	
	const int k_num_12 = 512;
	const int k_size_12 = 9;
	const int stride_12 = 1;
	const int k_depth_12 = im_depth_12;
	
	const int p1_12 = 1;
	const int p2_12 = 1;
	
	const int output_height_12 = (((im_height_12+(2*p1_12)) - sqrt(k_size_12))/stride_12) + 1;
	const int output_width_12 = (((im_width_12+(2*p2_12)) - sqrt(k_size_12))/stride_12) + 1;
	const int output_size_12 = output_height_12 * output_width_12;
	
	MatrixXd conv4_4_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_4_weights.csv");
	MatrixXd conv4_4_w = conv4_4_weights;
	
	MatrixXd conv4_4_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv4_4_biases.csv");
	VectorXd conv4_4_b(Map<VectorXd>(conv4_4_biases.data(), conv4_4_biases.cols()*conv4_4_biases.rows()));
	
	const int f_4 = 2;
	const int s_4 = 2;
	
	const int pp1_4 = 0;
	const int pp2_4 = 0;
	
	const int im_height_13 = ((output_height_12 - f_4 + 2 * pp1_4) / s_4) + 1;
	const int im_width_13 = ((output_width_12 - f_4 + 2 * pp2_4) / s_4) + 1;
	const int im_depth_13 = k_num_12;
	const int im_size_13 = im_height_13 * im_width_13;
	
	const int k_num_13 = 512;
	const int k_size_13 = 9;
	const int stride_13 = 1;
	const int k_depth_13 = im_depth_13;
	
	const int p1_13 = 1;
	const int p2_13 = 1;
	
	const int output_height_13 = (((im_height_13+(2*p1_13)) - sqrt(k_size_13))/stride_13) + 1;
	const int output_width_13 = (((im_width_13+(2*p2_13)) - sqrt(k_size_13))/stride_13) + 1;
	const int output_size_13 = output_height_13 * output_width_13;
	
	MatrixXd conv5_1_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_1_weights.csv");
	MatrixXd conv5_1_w = conv5_1_weights;
	
	MatrixXd conv5_1_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_1_biases.csv");
	VectorXd conv5_1_b(Map<VectorXd>(conv5_1_biases.data(), conv5_1_biases.cols()*conv5_1_biases.rows()));
	
	const int im_height_14 = output_height_13;
	const int im_width_14 = output_width_13;
	const int im_depth_14 = k_num_13;
	const int im_size_14 = im_height_14 * im_width_14;
	
	const int k_num_14 = 512;
	const int k_size_14 = 9;
	const int stride_14 = 1;
	const int k_depth_14 = im_depth_14;
	
	const int p1_14 = 1;
	const int p2_14 = 1;
	
	const int output_height_14 = (((im_height_14+(2*p1_14)) - sqrt(k_size_14))/stride_14) + 1;
	const int output_width_14 = (((im_width_14+(2*p2_14)) - sqrt(k_size_14))/stride_14) + 1;
	const int output_size_14 = output_height_14 * output_width_14;
	
	MatrixXd conv5_2_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_2_weights.csv");
	MatrixXd conv5_2_w = conv5_2_weights;
	
	MatrixXd conv5_2_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_2_biases.csv");
	VectorXd conv5_2_b(Map<VectorXd>(conv5_2_biases.data(), conv5_2_biases.cols()*conv5_2_biases.rows()));
	
	const int im_height_15 = output_height_14;
	const int im_width_15 = output_width_14;
	const int im_depth_15 = k_num_14;
	const int im_size_15 = im_height_15 * im_width_15;
	
	const int k_num_15 = 512;
	const int k_size_15 = 9;
	const int stride_15 = 1;
	const int k_depth_15 = im_depth_15;
	
	const int p1_15 = 1;
	const int p2_15 = 1;
	
	const int output_height_15 = (((im_height_15+(2*p1_15)) - sqrt(k_size_15))/stride_15) + 1;
	const int output_width_15 = (((im_width_15+(2*p2_15)) - sqrt(k_size_15))/stride_15) + 1;
	const int output_size_15 = output_height_15 * output_width_15;
	
	MatrixXd conv5_3_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_3_weights.csv");
	MatrixXd conv5_3_w = conv5_3_weights;
	
	MatrixXd conv5_3_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_3_biases.csv");
	VectorXd conv5_3_b(Map<VectorXd>(conv5_3_biases.data(), conv5_3_biases.cols()*conv5_3_biases.rows()));
	
	const int im_height_16 = output_height_15;
	const int im_width_16 = output_width_15;
	const int im_depth_16 = k_num_15;
	const int im_size_16 = im_height_16 * im_width_16;
	
	const int k_num_16 = 512;
	const int k_size_16 = 9;
	const int stride_16 = 1;
	const int k_depth_16 = im_depth_16;
	
	const int p1_16 = 1;
	const int p2_16 = 1;
	
	const int output_height_16 = (((im_height_16+(2*p1_16)) - sqrt(k_size_16))/stride_16) + 1;
	const int output_width_16 = (((im_width_16+(2*p2_16)) - sqrt(k_size_16))/stride_16) + 1;
	const int output_size_16 = output_height_16 * output_width_16;
	
	MatrixXd conv5_4_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_4_weights.csv");
	MatrixXd conv5_4_w = conv5_4_weights;
	
	MatrixXd conv5_4_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/conv5_4_biases.csv");
	VectorXd conv5_4_b(Map<VectorXd>(conv5_4_biases.data(), conv5_4_biases.cols()*conv5_4_biases.rows()));
	
	const int f_5 = 2;
	const int s_5 = 2;
	
	const int pp1_5 = 0;
	const int pp2_5 = 0;
	
	MatrixXd fc6_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/fc6_weights.csv");
	
	MatrixXd fc6_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/fc6_biases.csv");
	VectorXd fc6_b(Map<VectorXd>(fc6_biases.data(), fc6_biases.cols()*fc6_biases.rows()));
	
	MatrixXd fc7_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/fc7_weights.csv");
	
	MatrixXd fc7_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/fc7_biases.csv");
	VectorXd fc7_b(Map<VectorXd>(fc7_biases.data(), fc7_biases.cols()*fc7_biases.rows()));
	
	MatrixXd fc8_weights = load_csv_arma<MatrixXd>("../weights/VGG_19/fc8_weights.csv");
	
	MatrixXd fc8_biases = load_csv_arma<MatrixXd>("../weights/VGG_19/fc8_biases.csv");
	VectorXd fc8_b(Map<VectorXd>(fc8_biases.data(), fc8_biases.cols()*fc8_biases.rows()));
	
	const int im_num = 1;
	MatrixXd train = load_csv_arma<MatrixXd>("../inputs/VGG_19/production/VGG_19.csv");
	
    float gemm_time_total = 0.0;
    float run_time_total = 0.0;
    
    for(int i=0; i < im_num; i++)
    {   
        clock_t run_time_start = clock();    
        
        MatrixXd img;
        if ( train.rows() != 1 ) {
            img = train.block<1,im_size_1*im_depth_1>(i,0);
        }
        else {          
            img = train;
        }
        
        MatrixXd image = Map<Matrix<double, im_depth_1, im_size_1, RowMajor>>(img.data());
        
		MatrixXd conv1_1;
		double gemm_time_1;
		double offline_time_1;
		std::tie(conv1_1, gemm_time_1, offline_time_1) = convolve(image, im_size_1, im_height_1, im_width_1, im_depth_1, k_size_1, stride_1, conv1_1_b, p1_1, p2_1, conv1_1_w, output_size_1);
		
		MatrixXd relu1_1 = relu(conv1_1);
		
		MatrixXd conv1_2;
		double gemm_time_2;
		double offline_time_2;
		std::tie(conv1_2, gemm_time_2, offline_time_2) = convolve(relu1_1, im_size_2, im_height_2, im_width_2, im_depth_2, k_size_2, stride_2, conv1_2_b, p1_2, p2_2, conv1_2_w, output_size_2);
		
		MatrixXd relu1_2 = relu(conv1_2);
		
		MatrixXd pool1 = pool(relu1_2, f_1, s_1, output_width_2, output_height_2, pp1_1, pp2_1);
		
		MatrixXd conv2_1;
		double gemm_time_3;
		double offline_time_3;
		std::tie(conv2_1, gemm_time_3, offline_time_3) = convolve(pool1, im_size_3, im_height_3, im_width_3, im_depth_3, k_size_3, stride_3, conv2_1_b, p1_3, p2_3, conv2_1_w, output_size_3);
		
		MatrixXd relu2_1 = relu(conv2_1);
		
		MatrixXd conv2_2;
		double gemm_time_4;
		double offline_time_4;
		std::tie(conv2_2, gemm_time_4, offline_time_4) = convolve(relu2_1, im_size_4, im_height_4, im_width_4, im_depth_4, k_size_4, stride_4, conv2_2_b, p1_4, p2_4, conv2_2_w, output_size_4);
		
		MatrixXd relu2_2 = relu(conv2_2);
		
		MatrixXd pool2 = pool(relu2_2, f_2, s_2, output_width_4, output_height_4, pp1_2, pp2_2);
		
		MatrixXd conv3_1;
		double gemm_time_5;
		double offline_time_5;
		std::tie(conv3_1, gemm_time_5, offline_time_5) = convolve(pool2, im_size_5, im_height_5, im_width_5, im_depth_5, k_size_5, stride_5, conv3_1_b, p1_5, p2_5, conv3_1_w, output_size_5);
		
		MatrixXd relu3_1 = relu(conv3_1);
		
		MatrixXd conv3_2;
		double gemm_time_6;
		double offline_time_6;
		std::tie(conv3_2, gemm_time_6, offline_time_6) = convolve(relu3_1, im_size_6, im_height_6, im_width_6, im_depth_6, k_size_6, stride_6, conv3_2_b, p1_6, p2_6, conv3_2_w, output_size_6);
		
		MatrixXd relu3_2 = relu(conv3_2);
		
		MatrixXd conv3_3;
		double gemm_time_7;
		double offline_time_7;
		std::tie(conv3_3, gemm_time_7, offline_time_7) = convolve(relu3_2, im_size_7, im_height_7, im_width_7, im_depth_7, k_size_7, stride_7, conv3_3_b, p1_7, p2_7, conv3_3_w, output_size_7);
		
		MatrixXd relu3_3 = relu(conv3_3);
		
		MatrixXd conv3_4;
		double gemm_time_8;
		double offline_time_8;
		std::tie(conv3_4, gemm_time_8, offline_time_8) = convolve(relu3_3, im_size_8, im_height_8, im_width_8, im_depth_8, k_size_8, stride_8, conv3_4_b, p1_8, p2_8, conv3_4_w, output_size_8);
		
		MatrixXd relu3_4 = relu(conv3_4);
		
		MatrixXd pool3 = pool(relu3_4, f_3, s_3, output_width_8, output_height_8, pp1_3, pp2_3);
		
		MatrixXd conv4_1;
		double gemm_time_9;
		double offline_time_9;
		std::tie(conv4_1, gemm_time_9, offline_time_9) = convolve(pool3, im_size_9, im_height_9, im_width_9, im_depth_9, k_size_9, stride_9, conv4_1_b, p1_9, p2_9, conv4_1_w, output_size_9);
		
		MatrixXd relu4_1 = relu(conv4_1);
		
		MatrixXd conv4_2;
		double gemm_time_10;
		double offline_time_10;
		std::tie(conv4_2, gemm_time_10, offline_time_10) = convolve(relu4_1, im_size_10, im_height_10, im_width_10, im_depth_10, k_size_10, stride_10, conv4_2_b, p1_10, p2_10, conv4_2_w, output_size_10);
		
		MatrixXd relu4_2 = relu(conv4_2);
		
		MatrixXd conv4_3;
		double gemm_time_11;
		double offline_time_11;
		std::tie(conv4_3, gemm_time_11, offline_time_11) = convolve(relu4_2, im_size_11, im_height_11, im_width_11, im_depth_11, k_size_11, stride_11, conv4_3_b, p1_11, p2_11, conv4_3_w, output_size_11);
		
		MatrixXd relu4_3 = relu(conv4_3);
		
		MatrixXd conv4_4;
		double gemm_time_12;
		double offline_time_12;
		std::tie(conv4_4, gemm_time_12, offline_time_12) = convolve(relu4_3, im_size_12, im_height_12, im_width_12, im_depth_12, k_size_12, stride_12, conv4_4_b, p1_12, p2_12, conv4_4_w, output_size_12);
		
		MatrixXd relu4_4 = relu(conv4_4);
		
		MatrixXd pool4 = pool(relu4_4, f_4, s_4, output_width_12, output_height_12, pp1_4, pp2_4);
		
		MatrixXd conv5_1;
		double gemm_time_13;
		double offline_time_13;
		std::tie(conv5_1, gemm_time_13, offline_time_13) = convolve(pool4, im_size_13, im_height_13, im_width_13, im_depth_13, k_size_13, stride_13, conv5_1_b, p1_13, p2_13, conv5_1_w, output_size_13);
		
		MatrixXd relu5_1 = relu(conv5_1);
		
		MatrixXd conv5_2;
		double gemm_time_14;
		double offline_time_14;
		std::tie(conv5_2, gemm_time_14, offline_time_14) = convolve(relu5_1, im_size_14, im_height_14, im_width_14, im_depth_14, k_size_14, stride_14, conv5_2_b, p1_14, p2_14, conv5_2_w, output_size_14);
		
		MatrixXd relu5_2 = relu(conv5_2);
		
		MatrixXd conv5_3;
		double gemm_time_15;
		double offline_time_15;
		std::tie(conv5_3, gemm_time_15, offline_time_15) = convolve(relu5_2, im_size_15, im_height_15, im_width_15, im_depth_15, k_size_15, stride_15, conv5_3_b, p1_15, p2_15, conv5_3_w, output_size_15);
		
		MatrixXd relu5_3 = relu(conv5_3);
		
		MatrixXd conv5_4;
		double gemm_time_16;
		double offline_time_16;
		std::tie(conv5_4, gemm_time_16, offline_time_16) = convolve(relu5_3, im_size_16, im_height_16, im_width_16, im_depth_16, k_size_16, stride_16, conv5_4_b, p1_16, p2_16, conv5_4_w, output_size_16);
		
		MatrixXd relu5_4 = relu(conv5_4);
		
		MatrixXd pool5 = pool(relu5_4, f_5, s_5, output_width_16, output_height_16, pp1_5, pp2_5);
		
		MatrixXd fc6 = fully_connect(pool5, pool5.rows(), fc6_weights, fc6_b);
		
		MatrixXd relu6 = relu(fc6);
		
		MatrixXd fc7 = fully_connect(relu6, relu6.rows(), fc7_weights, fc7_b);
		
		MatrixXd relu7 = relu(fc7);
		
		MatrixXd fc8 = fully_connect(relu7, relu7.rows(), fc8_weights, fc8_b);
		
        clock_t run_time_end = clock();
        
        double run_time = (double) (run_time_end-run_time_start) / CLOCKS_PER_SEC;   
		run_time_total += (run_time - offline_time_1 - offline_time_2 - offline_time_3 - offline_time_4 - offline_time_5 - offline_time_6 - offline_time_7 - offline_time_8 - offline_time_9 - offline_time_10 - offline_time_11 - offline_time_12 - offline_time_13 - offline_time_14 - offline_time_15 - offline_time_16);
		gemm_time_total += 0.0 + gemm_time_1 + gemm_time_2 + gemm_time_3 + gemm_time_4 + gemm_time_5 + gemm_time_6 + gemm_time_7 + gemm_time_8 + gemm_time_9 + gemm_time_10 + gemm_time_11 + gemm_time_12 + gemm_time_13 + gemm_time_14 + gemm_time_15 + gemm_time_16;
		
		std::string name_1 = "../features/VGG_19/conv1_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_1, conv1_1);
		std::string name_2 = "../features/VGG_19/relu1_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_2, relu1_1);
		std::string name_3 = "../features/VGG_19/conv1_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_3, conv1_2);
		std::string name_4 = "../features/VGG_19/relu1_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_4, relu1_2);
		std::string name_5 = "../features/VGG_19/pool1_" + std::to_string(i) + ".csv";
		write_to_csv(name_5, pool1);
		std::string name_6 = "../features/VGG_19/conv2_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_6, conv2_1);
		std::string name_7 = "../features/VGG_19/relu2_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_7, relu2_1);
		std::string name_8 = "../features/VGG_19/conv2_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_8, conv2_2);
		std::string name_9 = "../features/VGG_19/relu2_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_9, relu2_2);
		std::string name_10 = "../features/VGG_19/pool2_" + std::to_string(i) + ".csv";
		write_to_csv(name_10, pool2);
		std::string name_11 = "../features/VGG_19/conv3_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_11, conv3_1);
		std::string name_12 = "../features/VGG_19/relu3_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_12, relu3_1);
		std::string name_13 = "../features/VGG_19/conv3_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_13, conv3_2);
		std::string name_14 = "../features/VGG_19/relu3_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_14, relu3_2);
		std::string name_15 = "../features/VGG_19/conv3_3_" + std::to_string(i) + ".csv";
		write_to_csv(name_15, conv3_3);
		std::string name_16 = "../features/VGG_19/relu3_3_" + std::to_string(i) + ".csv";
		write_to_csv(name_16, relu3_3);
		std::string name_17 = "../features/VGG_19/conv3_4_" + std::to_string(i) + ".csv";
		write_to_csv(name_17, conv3_4);
		std::string name_18 = "../features/VGG_19/relu3_4_" + std::to_string(i) + ".csv";
		write_to_csv(name_18, relu3_4);
		std::string name_19 = "../features/VGG_19/pool3_" + std::to_string(i) + ".csv";
		write_to_csv(name_19, pool3);
		std::string name_20 = "../features/VGG_19/conv4_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_20, conv4_1);
		std::string name_21 = "../features/VGG_19/relu4_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_21, relu4_1);
		std::string name_22 = "../features/VGG_19/conv4_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_22, conv4_2);
		std::string name_23 = "../features/VGG_19/relu4_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_23, relu4_2);
		std::string name_24 = "../features/VGG_19/conv4_3_" + std::to_string(i) + ".csv";
		write_to_csv(name_24, conv4_3);
		std::string name_25 = "../features/VGG_19/relu4_3_" + std::to_string(i) + ".csv";
		write_to_csv(name_25, relu4_3);
		std::string name_26 = "../features/VGG_19/conv4_4_" + std::to_string(i) + ".csv";
		write_to_csv(name_26, conv4_4);
		std::string name_27 = "../features/VGG_19/relu4_4_" + std::to_string(i) + ".csv";
		write_to_csv(name_27, relu4_4);
		std::string name_28 = "../features/VGG_19/pool4_" + std::to_string(i) + ".csv";
		write_to_csv(name_28, pool4);
		std::string name_29 = "../features/VGG_19/conv5_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_29, conv5_1);
		std::string name_30 = "../features/VGG_19/relu5_1_" + std::to_string(i) + ".csv";
		write_to_csv(name_30, relu5_1);
		std::string name_31 = "../features/VGG_19/conv5_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_31, conv5_2);
		std::string name_32 = "../features/VGG_19/relu5_2_" + std::to_string(i) + ".csv";
		write_to_csv(name_32, relu5_2);
		std::string name_33 = "../features/VGG_19/conv5_3_" + std::to_string(i) + ".csv";
		write_to_csv(name_33, conv5_3);
		std::string name_34 = "../features/VGG_19/relu5_3_" + std::to_string(i) + ".csv";
		write_to_csv(name_34, relu5_3);
		std::string name_35 = "../features/VGG_19/conv5_4_" + std::to_string(i) + ".csv";
		write_to_csv(name_35, conv5_4);
		std::string name_36 = "../features/VGG_19/relu5_4_" + std::to_string(i) + ".csv";
		write_to_csv(name_36, relu5_4);
		std::string name_37 = "../features/VGG_19/pool5_" + std::to_string(i) + ".csv";
		write_to_csv(name_37, pool5);
		std::string name_38 = "../features/VGG_19/fc6_" + std::to_string(i) + ".csv";
		write_to_csv(name_38, fc6);
		std::string name_39 = "../features/VGG_19/relu6_" + std::to_string(i) + ".csv";
		write_to_csv(name_39, relu6);
		std::string name_40 = "../features/VGG_19/fc7_" + std::to_string(i) + ".csv";
		write_to_csv(name_40, fc7);
		std::string name_41 = "../features/VGG_19/relu7_" + std::to_string(i) + ".csv";
		write_to_csv(name_41, relu7);
		std::string name_42 = "../features/VGG_19/fc8_" + std::to_string(i) + ".csv";
		write_to_csv(name_42, fc8);
    }

    cout << "-----------------------------" << endl;

    float avg_run_time = 0.0;            
    avg_run_time = run_time_total / im_num;
    cout << "average online run time: " << avg_run_time << endl;

    float avg_gemm_time = 0.0;
    avg_gemm_time = gemm_time_total / im_num;
    cout << "average total time for GEMM: " << avg_gemm_time << endl;

    return 0; 
}
